...
const resultConteiner = document.querySelector("#result-container");
...
function showOrHideResults() {
    ...
    resultConteiner.classList.toggle("hide");
}
//Inicialização
createTable(data);

//Eventos
[heightImput, weightImput].forEach((el) => {
...
calcBtn.addEventListener("click", (e) => {
    ...
    let info;
    data.forEach((item) => {
        if(imc >= item.min && imc <= item.max) info = item.info;
    });
    if(!info) return;
